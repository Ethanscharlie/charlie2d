...
void Scene::update()
{
    ...
    if (1.0f / deltaTime < 1000) std::cout << "FPS: " << 1.0f / deltaTime << std::endl;
            ...
            if (entity->toDestroy) {
                ...
                if (component->standardUpdate) {
                    if (component->useLayer) {
                        ...
                        //SDL_Rect renderBox;
                        //Vector2f renderPos = entity->box->getBox().getCenter() * scaler;
                        //renderBox.x = renderPos.x - (scaler * GameManager::camera.x - GameManager::windowWidth  / 2); 
                        //renderBox.y = renderPos.y - (scaler * GameManager::camera.y - GameManager::windowHeight / 2);   

                        //renderBox.w = entity->box->getSize().x * scaler + 1;
                        //renderBox.h = entity->box->getSize().y * scaler + 1;

                        //if (renderBox.x > 0 - renderBox.w && renderBox.x < GameManager::windowWidth + renderBox.w && renderBox.y > 0-renderBox.h && renderBox.y < GameManager::windowHeight + renderBox.h) {
                        layeredComponents.push_back(component);
                        //}
                    } else {
                        //float scaler = GameManager::screen_change_scale * GameManager::cameraZoom;
                        ...
                        //if (renderBox.x > 0 && renderBox.x < GameManager::windowWidth && renderBox.y > 0 && renderBox.y < GameManager::windowHeight) {
...
void Scene::loadLDTK(std::string file) {
    ...
    for (auto it = layerInstances.rbegin(); it != layerInstances.rend(); ++it) { 
        ...
        if (layer["__type"] == "Tiles") {
            ...
            Entity* layerObject = createEntity(layer["__identifier"]);
