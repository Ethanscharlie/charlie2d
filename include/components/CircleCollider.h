...
public:
   ...
   */
  void start() override { radius = entity->box.size.y / 2; }

  /**
   * \brief Keeps sets the radius? Probably needs to be fixed
   */
  void update(float deltaTime) override { radius = entity->box.size.y / 2; }
  ...
  bool checkCircleCollision(CircleCollider *col) {
    float distance = std::sqrt(
        std::pow(col->entity->box.getCenter().x - entity->box.getCenter().x,
                 2) +
        std::pow(col->entity->box.getCenter().y - entity->box.getCenter().y,
                 2));
